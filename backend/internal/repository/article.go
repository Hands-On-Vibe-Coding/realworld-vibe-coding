...
func (r *ArticleRepository) GetArticles(limit, offset int, tag, author, favorited string) ([]model.Article, int, error) {
	...
	// Get articles
	articlesQuery := `
		SELECT DISTINCT a.id, a.slug, a.title, a.description, a.body, a.author_id, a.created_at, a.updated_at, a.favorites_count
...
func (r *ArticleRepository) GetFeedArticles(limit, offset, userID int) ([]model.Article, int, error) {
	...
	// Get articles
	articlesQuery := `
		SELECT a.id, a.slug, a.title, a.description, a.body, a.author_id, a.created_at, a.updated_at, a.favorites_count
	` + baseQuery + `
		ORDER BY a.created_at DESC
		LIMIT ? OFFSET ?
	`
